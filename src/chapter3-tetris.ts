...
namespace CCTJ {
  ...
  function playTetrisRound(
    ...
    while (!failed && game.runtime() - start < duration) {
      let now = game.runtime();
      let remaining = Math.ceil((duration - (now - start)) / 1000);
  ...
  export function playChapter3(): void {
    ...
    setupRichScene(Art.bg_moscow, Art.pajitnov, 32, 84, 128, 84); // grey bg — austere office
    ...
    npcSay("PAJITNOV", "I made a little puzzle instead.");
    npcSay(
      "PAJITNOV",
    ...
    npcSay("PAJITNOV", "But I'm not so sure about it. You try...");

    clearAllSprites();

    // ── Round 1: Pentomino version ─────────────────────────
    say([
      "ROUND 1: Pentominoes! 5-square shapes. No rotation. Survive 10 seconds!",
    ]);
    let r1 = playTetrisRound(1, true);

    setupRichScene(Art.bg_moscow, Art.pajitnov, 32, 84, 128, 84);
    if (r1) {
      npcSay(
        "PAJITNOV",
      ...
      npcSay("PAJITNOV", "See? It's too hard! What do you think?");
    }
    ...
    npcSay("PAJITNOV", "Much simpler! And you could rotate them!");
    npcSay(
      "PAJITNOV",
